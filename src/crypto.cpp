...
#ifdef USE_GCRYPT
#include "../include/gcrypt-openssl-wrapper.h"
#include "../include/sha1-git.h"
#else
...
void rc4_setup(struct rc4_state * s, const unsigned char * key, int32_t length)
{
	int32_t i, j, k, *m;
...
void rc4_crypt(struct rc4_state * s, unsigned char * data, int32_t length)
{
	int32_t i, x, y, *m;
...
unsigned long calc_crc(unsigned char * buf, int32_t len)
{
...
unsigned long calc_crc_plain(unsigned char * buf, int32_t len)
{
...
int32_t is_arp(void * wh, size_t len)
{
...
int32_t is_wlccp(void * wh, size_t len)
{
	size_t wlccpsize = 58;
...
int32_t is_qos_arp_tkip(void * wh, int32_t len)
{
	...
	int32_t qosarpsize = (24 + 2) + 8 + (8 + (8 + 10 * 2)) + 8 + 4; // 82 in total
	...
	void * clear, int32_t * clen, int32_t * weight, unsigned char * wh, size_t len)
{
	...
	else /* IP */
	{
		...
		if (!weight)
		{
			*clen = (int32_t) (ptr - ((unsigned char *) clear));
			return 1;
		}
#if 1
...
int32_t init_michael(struct Michael * mic, const unsigned char key[8])
{
...
int32_t michael_append_byte(struct Michael * mic, unsigned char byte)
{
...
int32_t michael_remove_byte(struct Michael * mic, const unsigned char bytes[4])
{
	if (mic->nBytesInM == 0)
	{
		// Clear the buffer
...
int32_t michael_append(struct Michael * mic, unsigned char * bytes, int32_t length)
{
	while (length > 0)
	{
...
int32_t michael_remove(struct Michael * mic, unsigned char * bytes, int32_t length)
{
	while (length >= 4)
	{
...
int32_t michael_finalize(struct Michael * mic)
{
...
int32_t michael_finalize_zero(struct Michael * mic)
{
	// Append the minimum padding
	michael_append_byte(mic, 0);
	michael_append_byte(mic, 0);
...
int32_t calc_tkip_mic_key(unsigned char * packet, int32_t length, unsigned char key[8])
{
	...
	michael_remove(&mic, message, (int32_t) (ptr - message));
...
int32_t calc_tkip_mic_skip_eiv(unsigned char * packet,
		                   int32_t length,
						   unsigned char ptk[80],
						   unsigned char value[8])
{
...
static inline void XOR(unsigned char * dst, unsigned char * src, int32_t len)
{
	int32_t i;
...
int32_t encrypt_ccmp(unsigned char * h80211,
				 ...
				 unsigned char PN[6])
{
	int32_t is_a4, i, z, blocks, is_qos;
	int32_t data_len, last, offset;
	...
	for (i = 1; i <= blocks; i++)
	{
		int32_t n = (last > 0 && i == blocks) ? last : 16;
